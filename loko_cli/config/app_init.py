...
GATEWAY_DS = DockerService(name="gw", image="lokoai/loko-gateway:0.0.4-dev", ports={"8080": "19999"},
                           environment=dict(AUTOSCAN=False,
                                            RULES=[{"name": "orchestrator", "host": "orchestrator", "port": 8888,
                                                    "type": "orchestrator","scan":True},
                                                   {"name": "predictor", "host": "predictor", "port": 8080,
                                                    "type": "custom","scan":False}]))
ORCHESTRATOR_DS = DockerService(name="orchestrator", image="lokoai/loko-orchestrator:0.0.3-dev",
...
TEMPORARY_MAPPING = dict(predictor=DockerService(name="predictor", image="lokoai/ds4biz-predictor-dm:0.0.3-dev",
                                                 volumes=[
